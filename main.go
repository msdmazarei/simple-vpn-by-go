...
they could sniff. we could handle this by TUN/TAP(ref: https://en.wikipedia.
org/wiki/TUN/TAP) that provides virtual interface to user-space program,
there is another way that could helps us is using iptables ( by REDIRECT and
write our app by RAW_SOCKET)
[ref: http://man7.org/linux/man-pages/man7/raw.7.html].
or write custem linux driver for network and at worse case scencario (by passing
linux kernel network stack) we could handle traffic manually( by DPDK(
ref: https://www.dpdk.org/).  the final solution depends on many
...
this project contains below files:
	...
	3. main.go	(that inits TUN device and capture packets then sends them over
		network to target machine)

[flow:]
-------

 1. parse config (by calling ParseConfig)
 2. init TUN device and configure it and make it up (user should set address and
	define routes manually to his device)
 3. start a thread (go routine) to capture packets from TUN device and try to
	encrypt and to send them to target machine over network using UDP protocol.
 4. start a thead (go routine) to listen to network and receive packets and try
	to decrypt and push them to TUN device.

[why TUN device is better than TAP for this code?]
--------------------------------------------------

	as far as I know TUN device packets pass through linux routing, so already
	it could handle routing and many unnessecry
	challenges, but TAP is a layer 2 device and if I used it may force me to
	handle parse ethernet frame and routing mannually!

[Improvment]
------------

	1. Performance
		1.1 it is better to use SO_REUSEPORT option for UDP sockets(becuase it
			allows to multiple thread/process to listen on same port) .
			[https://lwn.net/Articles/542629/]
		1.2 it is better to run multiple thread(go routine) to increase capture
